...
{ 
	...
	printf("Time elapsed for cuda parallel code is: %f ms\n",time);
}
...
int main(int argc, char* argv[])
{
	...
	A.height = B.height = C.height = TOTAL_SIZE;
	A.width = B.width = C.width = TOTAL_SIZE;
	...
	C.elements = new float[C.width*C.height];
		...
		for (int j = 0; j < TOTAL_SIZE; j++) {
			A.elements[i*TOTAL_SIZE+j] = i*j;
			B.elements[i*TOTAL_SIZE+j] = (TOTAL_SIZE-i)*(TOTAL_SIZE-j);
			C.elements[i*TOTAL_SIZE+j] = 0;
		}
	}
